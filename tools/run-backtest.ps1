...
param(
    ...
    [double]$RsiLower = 20,                       # 売りを見送る／決済する RSI
    [int]$RsiExit     = 0,                        # RSI が行きすぎたら決済する 0=しない 1=する
...
Write-Host "設定: $ini"
Write-Host "実行: $Symbol $Period  $From 〜 $To  決済方式=$Exit  段階=$StagedMode  RSI=$RsiUpper/$RsiLower  RSI決済=$RsiExit  識別名=$Tag"
